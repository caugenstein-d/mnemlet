...
class MemoriaConfig:
    """Memoria configuration with sensible defaults."""

    def __init__(
        self,
        server_host: str = "127.0.0.1",
        server_port: int = 4050,
        data_dir: Path = Path.home() / ".memoria",
        ...
        self.server_host = os.environ.get("MEMORIA_HOST", server_host)
        self.server_port = int(os.environ.get("MEMORIA_PORT", server_port))

        env_data_dir = os.environ.get("MEMORIA_DATA_DIR")
        if env_data_dir:
            data_dir = Path(env_data_dir).expanduser()

        self.data_dir = data_dir
        self.sqlite_path = sqlite_path or self.data_dir / "memoria.db"
    ...
    def from_toml(cls, path: str) -> "MemoriaConfig":
        ...
        return cls(
            ...
            data_dir=Path(storage.get("data_dir", "~/.memoria")).expanduser(),
...
def load_config(path: Optional[str] = None) -> MemoriaConfig:
    """Load configuration from a TOML path or return defaults."""
    if path:
        return MemoriaConfig.from_toml(path)
